# Home Service Studios (homeservicestudios.com)
...
and had the old origin hard coded instead. Canonical tags, `og:url`, the sitemap, `robots.txt`
and the JSON-LD record all derive from `SITE`, and until this moved every page served from
homeservicestudios.com was telling Google the authoritative copy lived at
...
**Hosting moved to GitHub Pages on 2026-08-28** (commit `f8774d5`). The live site is the
`gh-pages` branch of `verysillydev/hsspage`, served at homeservicestudios.com via a `CNAME`
that `deploy_github.sh` writes on every deploy. `main` stays the source; `gh-pages` is build
output only and gets force-pushed fresh each time.

**`./deploy.sh` is the old Vercel path and does not work on this machine.** There is no Vercel
CLI auth here (`~/.local/share/com.vercel.cli/auth.json` does not exist), so it fails. It is kept
only in case hosting moves back. Use `./deploy_github.sh`.
...
**Decorative art ships as WebP, not PNG.** Converted 2026-09-03, when the site was carrying about
1.2MB of avoidable image weight. PNG is the wrong container for photographic and scanned
material, and these were all photographic:
...
Two things to keep in mind if you touch these:
...
call" does not fit next to the wordmark and both links on a 390px screen.

**The CTA must not carry an `aria-label`.** It used to be hard set to the long label, so below
560px the button displayed "Contact" and announced "Start a project". That is Lighthouse's
`label-content-name-mismatch`, and for a voice-control user saying "click Contact" it simply does
nothing. Only one of the two spans is ever displayed (the other is `display:none`, so it is
excluded from the accessible name), which means letting the name come from the content keeps the
visible and announced labels identical at every width. Both labels are descriptive enough to pass
the `link-text` audit that "Start" on its own failed. Removed 2026-09-03.
...
`reassure(page)` takes the page as an argument and picks from `REASSURE_VARIANTS`. It used to
render one identical sentence in all five places it appears, and a sentence insisting a human is
involved, reading verbatim on every page, is precisely the tell the rest of this document is
about. `/packages` calls it twice, so it has its own second variant (`packages-terms`).
...
this page.** `/contact/` used to post to a Vercel serverless function at `deploy/api/contact.js`,
built from `api_contact.js`. GitHub Pages is static and cannot execute it, so from 2026-08-28 to
2026-09-03 every submission got `405 Not Allowed` and **not one enquiry was delivered**. Nothing
in the source looked wrong; only a live POST showed it. Test the real endpoint after any hosting
...
Three things moved from the server into the client with it, and must stay: the subject line is
still built as `Company (City) - Trade - Budget` so the inbox stays sortable, `_replyto` is still
...
environment. If hosting ever returns to Vercel, point `FORM_ENDPOINT` back at `/api/contact` and
it works again; the `TRADE_GROUPS` / `TRADES` parity rule below still applies to that path.
...
- **A sticky action bar on phones**, contact and start-a-project, hidden from 760px up.
  **It overlapped the hero headline until 2026-09-03.** `.hero-media` is `min-height:100dvh` with
  ...
  bar started at 789px, so "does nothing.", the payoff line of the headline, was covered. The
  `body{padding-bottom:54px}` that already existed does not help, because it moves the end of the
...
- **Two crew bios.** `member_card()` renders an optional `bio` field, added 2026-09-03. Yoni's is
  filled, adapted from the paragraph already published on `/our-work` so the two pages describe
  him in the same terms. Paloma and Sergy have a name and a title only. A card with nothing but
...
One `ProfessionalService` block, identical on every page, built in `JSON_LD`. Two rules:

- **The URL and the email come from `SITE` and `EMAIL`, never a literal.** Hard coding them is
  exactly how this block kept naming yoniverseproductions.com after the site had moved.
- **The offer catalogue is generated from `data/packages.json`**, the same file `/packages`
  renders from, so the six prices in a search result cannot disagree with the six on the page.
